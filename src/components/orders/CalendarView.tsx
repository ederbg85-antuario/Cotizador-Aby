...
export default function CalendarView({ appointmentDates = [], onDateSelect }: CalendarViewProps) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-lav-100 shadow-soft p-7 md:p-8">
      <div className="flex items-center justify-between mb-7">
        <h3 className="font-display text-xl text-warm-800 capitalize">
          {format(current, "MMMM yyyy", { locale: es })}
        </h3>
        <div className="flex gap-2">
          <button
            onClick={() => setCurrent(subMonths(current, 1))}
            className="w-10 h-10 flex items-center justify-center hover:bg-mauve-100 text-warm-500 hover:text-mauve-700 rounded-xl transition-colors"
          >
            <ChevronLeft size={16} />
          </button>
          <button
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              {format(day, "d")}
              {appt && !tod && (
                <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 bg-mauve-600 rounded-full" />
              )}
            </button>
          );
        })}
      ...
      <div className="mt-7 pt-5 border-t border-lav-100 flex items-center gap-6 text-xs text-warm-500">
