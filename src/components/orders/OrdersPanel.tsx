    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
        <div>
          ...
          <h1 className="font-display text-4xl md:text-5xl text-warm-800 tracking-tight leading-tight">
            Mis <span className="italic text-brand-gradient">pedidos</span>
          </h1>
          <p className="text-sm text-warm-500 mt-2">Gestiona tus órdenes y citas con elegancia</p>
        </div>
        <Link
          href="/"
          className="btn-primary inline-flex items-center gap-2 px-5 py-3 font-semibold text-sm rounded-2xl"
        >
          <Plus size={16} />
          Nuevo Pedido
        </Link>
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4">
        {[
          ...
          <div key={label} className="bg-white rounded-2xl border border-lav-100 shadow-soft px-5 py-4 flex items-center gap-4">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${color} flex items-center justify-center text-white font-display font-bold text-sm`}>
              {value}
            </div>
            <div>
              <p className={`text-xs font-semibold ${text}`}>{label}</p>
              <p className="text-[11px] text-warm-400 mt-0.5">en total</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {filteredOrders.length === 0 ? (
        <div className="bg-white rounded-3xl border border-lav-100 shadow-soft py-16 px-8 text-center">
          <div className="max-w-sm mx-auto">
            <div className="w-20 h-20 mx-auto mb-6 bg-brand-gradient-soft rounded-3xl flex items-center justify-center text-4xl float">
              💅
            </div>
            <h2 className="font-display text-2xl text-warm-800 mb-3">Sin pedidos por ahora</h2>
            <p className="text-warm-500 text-sm mb-7 leading-relaxed">
              {selectedDate
            ...
            <Link
              href="/"
              className="btn-primary inline-flex items-center gap-2 px-7 py-3 font-semibold text-sm rounded-2xl"
            >
