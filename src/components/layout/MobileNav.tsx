...
export default function MobileNav() {
  ...
  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 glass border-t border-lav-100/70 z-50 pb-[env(safe-area-inset-bottom)]">
      <div className="flex justify-around items-center h-16">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              {active && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-10 h-0.5 bg-brand-gradient rounded-b-full" />
              )}
